...
from mae_abl.vision_transformer import PatchEmbed, Block, CBlock
...
class MaskedAutoencoderConvViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 embed_dim=500, depth=3, num_heads=20,
                 decoder_embed_dim=500, decoder_depth=3, decoder_num_heads=20,
    ...
    def forward_encoder(self, x, mask):
        ...
        for blk in self.blocks2:
            x = blk(x)
        stage2_embed = self.stage2_output_decode(x).flatten(1).unsqueeze(1)

        # x = self.patch_embed3(x)#.squeeze(0))
        x = self.patch_embed3(x).unsqueeze(0)
