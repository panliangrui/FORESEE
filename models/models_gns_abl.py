...
from models.crossViT2 import CrossScaleFusionTransformer
...
class fusion_model_mae(nn.Module):
    def __init__(self, dict, out_classes, dropout=0.2, train_type_num=3):
        ...
        #
        # # elif mae_ == 'FCMAE':
        # self.mae_FCMAE = ConvNeXtV2(depths=[2, 2, 6, 6], dims=[600, 600, 600, 600])
        #
        #
        # # elif mae =='MAGE':
    ...
    def forward(self, all_thing, train_use_type=None, use_type=None, in_mask=[], in_mask1=[], in_mask2=[], mix=False, mae=None, views=None):
        ...
        if 'img' in data_type:
            with torch.no_grad():
                x_img = self.wsi(x_img_256, x_img_512, x_img_1024, edge_index_img_256, edge_index_img_512, edge_index_img_1024, views)
        ...
        if len(train_use_type) > 1:
            if use_type == train_use_type:
                ...
                #     mae_x = None
                # mae_x = self.mae(pool_x, mask, mask1, mask2).squeeze(0)
                mae_x = self.mae(pool_x, mask, mask2).squeeze(0)
                # mae_x = self.mae(pool_x, mask).squeeze(0)
                ...
                mae_x = self.mae(tmp_x, mask, mask2).squeeze(0)
                fea_dict['mae_out'] = mae_x
