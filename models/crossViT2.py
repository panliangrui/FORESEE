...
class Mlp(nn.Module):
    def __init__(self, hidden_size=500, dropout_rate=0):
...
class CrossScaleFusionTransformer(nn.Module):
    def __init__(self, dict, dropout_rate=0):
        super(CrossScaleFusionTransformer, self).__init__()
        ###############
        self.img_node = dict['img_node']
        self.img_node_1024 = dict['img_node_1024']
        ...
        self.img_gnn3 = SAGEConv(in_channels=self.img_node_1024, out_channels=self.img_out)
        ...
        #level2
        self.SA_B2_1 = SelfAttention(embed_dim=self.embed, num_heads=4, dropout_rate=dropout_rate)
        ...
        self.SA_B2_2 = SelfAttention(embed_dim=self.embed, num_heads=4, dropout_rate=dropout_rate)
        ...
        self.SA_B2_3 = SelfAttention(embed_dim=self.embed, num_heads=4, dropout_rate=dropout_rate)
        ...
        #level3
        self.SA_B3_1 = SelfAttention(embed_dim=self.embed, num_heads=4, dropout_rate=dropout_rate)
        ...
        self.SA_B3_2 = SelfAttention(embed_dim=self.embed, num_heads=4, dropout_rate=dropout_rate)
        ...
        self.SA_B3_3 = SelfAttention(embed_dim=self.embed, num_heads=4, dropout_rate=dropout_rate)
    ...
    def forward(self, x_img_256, x_img_512, x_img_1024, edge_index_img_256, edge_index_img_512, edge_index_img_1024, views): #
        ##############################解析数据
        # x_img = x.x_img
        ...
        mlp2_1 = mlp2_1 + normal2_1 #atten2_1 +

        atten2_2 = self.SA_B2_2(x2)
        # atten2_2 = self.dropout(atten2_2)
        normal2_2 = self.ffn_norm2_2(atten2_2)
        # normal2_2 = self.gcn2_2(normal2_2, x_edge)
        mlp2_2 = self.Mlp2_2(normal2_2)
        mlp2_2 = mlp2_2 + normal2_2 #atten2_2 +

        atten2_3 = self.SA_B2_3(x3)
        ...
        mlp2_3 = mlp2_3 + normal2_3

        if views == '256':
            fused_graph2 = self.dropout(mlp2_1)

        elif views == '512':
            fused_graph2 = self.dropout(mlp2_2)

        elif views == '1024':
            fused_graph2 = self.dropout(mlp2_3)


        elif views == '256_512':
            ########小视场和中视场
            fused_graph2_1 = self.down2(torch.cat([mlp2_1, mlp2_2], dim=1))
            fused_graph2_1 = self.down2_1(fused_graph2_1)
            fused_graph2 = self.dropout(fused_graph2_1)  # .transpose(1, 2)

        elif views == '256_1024':
            ########小视场和大视场
            fused_graph2_1 = self.down2(torch.cat([mlp2_1, mlp2_3], dim=1))
            fused_graph2_1 = self.down2_1(fused_graph2_1)
            fused_graph2 = self.dropout(fused_graph2_1)  # .transpose(1, 2)

        elif views == '512_1024':
            ########小视场和大视场
            fused_graph2_1 = self.down2(torch.cat([mlp2_2, mlp2_3], dim=1))
            fused_graph2_1 = self.down2_1(fused_graph2_1)
            fused_graph2 = self.dropout(fused_graph2_1)  # .transpose(1, 2)

        elif views == '256_512_1024':
            ########小视场和中视场和大视场
            # 将三个特征图进行维度变换
            ...
            mlp3_1 = mlp3_1 + normal3_1  # atten3_1 +
            ...
            mlp3_2 = mlp3_2 + normal3_2  # atten3_2 +
            ...
            mlp3_3 = mlp3_3 + normal3_3  # atten3_3 +
