...
class fusion_model_mae(nn.Module):
    def __init__(self, dict, out_classes, dropout=0.0, train_type_num=3):
        ...
        # self.mae_MAE = PretrainVisionTransformer(encoder_embed_dim=out_classes, decoder_num_classes=out_classes,
        #                                          decoder_embed_dim=out_classes, encoder_depth=1, decoder_depth=1,
        #                                          train_type_num=train_type_num)
        # self.mae_DMAE = MaskedAutoencoderConvViT(
        ...
        #     aligned_feature_projection_dim=[500, 1000])
        ...
        #
        #
        # self.mae_PUT = PatchVQGAN()
        #
        #
        # self.mae_FCMAE = ConvNeXtV2(depths=[2, 2, 6, 6], dims=[500, 500, 500, 500])
        #
        #
        #
        # self.mae_MAGE = MaskedGenerativeEncoderViT()

        # self.mae_CONVMAE = MaskedAutoencoderConvViT()
    ...
    def forward(self, all_thing, train_use_type=None, use_type=None, in_mask=[], in_mask1=[], in_mask2=[], mix=False, mae=None):
        ...
        if len(train_use_type) > 1:
            if use_type == train_use_type:
                ...
                if mae == 'MAE':
                    mae_x = self.mae_MAE(pool_x, mask).squeeze(0)
                elif mae == 'PUT':
                    mae_x = self.mae_PUT(pool_x, mask).squeeze(0)
                elif mae == 'FCMAE':
                    mae_x = self.mae_FCMAE(pool_x, mask).squeeze(0)
                elif mae == 'DMAE':
                    with torch.no_grad():
                        latents_teacher = self.mae_DMAE.forward_encoder_customized(pool_x)
                        # teacher_prediction = self.mae.module.forward_decoder(latents_teacher)
                    loss_distillation_embedding, mae_x = self.mae_DMAE(pool_x, mask, latents_teacher)
                    loss_distillation_embedding += loss_distillation_embedding
                    mae_x = mae_x.squeeze(0)
                    # mae_x = self.mae_DMAE(pool_x, mask).squeeze(0)
                elif mae == 'MAGE':
                    mae_x = self.mae_MAGE(pool_x, mask).squeeze(0)
                elif mae == 'CONVMAE':
                    mae_x = self.mae_CONVMAE(pool_x, mask).squeeze(0)
                elif mae == 'our':
            ...
            # save_fea['after_mae'] = pool_x.cpu().detach().numpy()
            if mix:
