...
from models.models_gns_abl import fusion_model_mae
...
def prediction(all_data, v_model, val_id, patient_and_time, patient_sur_type, args):
    ...
    with torch.no_grad():
        for i_batch, id in enumerate(val_id):
            ...
            out_pre, out_fea, out_att, _ = v_model(graph, args.train_use_type, use_type_eopch, mix=args.mix, views=args.views)
            lbl_pred = out_pre[0]
...
def train_a_epoch(model, train_data, all_data, patient_and_time, patient_sur_type, batch_size, optimizer, epoch, format_of_coxloss, args):
    global survtime_mut_cnv, status_mut_cnv
    model.train()

    lbl_pred_each = None
    lbl_pred_img_each = None
    lbl_pred_rna_each = None
    lbl_pred_mut_cnv_each = None
    batch_feature = None
    ...
    for i_batch, id in enumerate(train_data):
        ...
        if len(args.train_use_type) == 1:
            ...
            graph = all_data[id].to(device)
            with torch.cuda.amp.autocast():
                out_pre, out_fea, out_att, fea_dict = model(graph, use_type_eopch, use_type_eopch, mask, mask1, mask2, mix=args.mix,views=args.views)
                lbl_pred = out_pre[0]
        ...
        if len(args.train_use_type) == 1 and args.train_use_type[0] not in all_data[id].data_type:
            ...
            if 'mut_cnv' in use_type_eopch and len(args.train_use_type) != 1:
                train_pre_time_mut_cnv[id] = out_pre[1][use_type_eopch.index('mut_cnv')].cpu().detach().numpy()
                survtime_mut_cnv.append(patient_and_time[id])
                status_mut_cnv.append(patient_sur_type[id])
                if lbl_pred_mut_cnv_each == None:
                    lbl_pred_mut_cnv_each = out_pre[1][use_type_eopch.index('mut_cnv')]
                else:
                    lbl_pred_mut_cnv_each = torch.cat([lbl_pred_mut_cnv_each, out_pre[1][use_type_eopch.index('mut_cnv')]])
        ...
        if iter % batch_size == 0 or i_batch == len(train_data) - 1:
            ...
            if format_of_coxloss == 'one':
                ...
                if lbl_pred_mut_cnv_each != None:
                    mut_cnv_loss_surv = args.mut_cnv_cox_loss_factor * _neg_partial_log(lbl_pred_mut_cnv_each, survtime_mut_cnv,
                                                                                status_mut_cnv)
                    loss_surv += mut_cnv_loss_surv
            else:
    ...
    if len(args.train_use_type) != 1:
        ...
        if 'mut_cnv' in args.train_use_type:
            t_train_ci_mut_cnv = get_val_ci(train_pre_time_mut_cnv, patient_and_time, patient_sur_type)

    return all_loss, t_train_ci, t_train_ci_img, t_train_ci_rna, t_train_ci_mut_cnv
...
def main(args):
    ...
    joblib.dump(all_gnn_time, '../results_abl/UCEC/our/' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')
    joblib.dump(all_each_model_time, '../results_abl/UCEC/our/' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')
    joblib.dump(all_all_ci, '../results_abl/UCEC/our/' + 'all' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')


def get_params():
    parser = argparse.ArgumentParser()
    parser.add_argument('-config', type=str, default='E:\\multi-mode\\Configs\\BRCA.yml', help='yml path')
    ...
    parser.add_argument("--train_use_type", type=list, default=['img'],    #['img', 'rna', 'mut_cnv']
    ...
    parser.add_argument("--batch_size", type=int, default=20, help="Data volume of model training once")
    ...
    parser.add_argument("--views", type=str, default='256', help="Experimental details")
    parser.add_argument("--all_cox_loss_factor", type=int, default='5', help="Experimental details")
