...
def tiling(slide_filepath, magnification, patch_size, scale_factor=32, tissue_thresh=0.35, method='adaptive',
           overview_level=5,
           coord_dir=None, overview_dir=None, mask_dir=None, patch_dir=None, filename=None, model_final=None,
           thumbnail_dir=None):
    feature = {}
...
def run(args):
    ...
    # model_final = ctranspath()
    # model_final = model_final.to(device)
    # model_final.head = nn.Identity()
    # td = torch.load(r'../ctranspath.pth')
    # model_final.load_state_dict(td['model'], strict=False)
    ...
    all_slide_name = joblib.load('Q:\\COAD_WSI\\coad\\coad_patients.pkl')
...
def main():
    ...
    parser.add_argument('--slide_dir', type=str, default='Q:\\COAD_WSI\\process_wsi')
    parser.add_argument('--save_dir', type=str, default='Q:\\COAD_WSI\\coad\\128')
