...
from models.models_gns_mae_abl import fusion_model_mae
    ...
    with torch.cuda.amp.autocast():
        with torch.no_grad():
            for i_batch, id in enumerate(val_id):
                ...
                out_pre, out_fea, out_att, _ = v_model(graph, args.train_use_type, use_type_eopch, mix=args.mix, mae = args.mae)
                lbl_pred = out_pre[0]
        ...
        for i_batch, id in enumerate(train_data):
            ...
            if len(args.train_use_type) == 1:
                ...
                if args.train_use_type[0] in all_data[id].data_type:
                    graph = all_data[id].to(device)
                    with torch.cuda.amp.autocast():
                        out_pre, out_fea, out_att, fea_dict = model(graph, args.train_use_type, args.train_use_type, mix=args.mix, mae=args.mae)
                        lbl_pred = out_pre[0]
                        use_type_eopch = args.train_use_type
                        num_of_model = 1
            else:
                ...
                # with torch.cuda.amp.autocast():
                graph = all_data[id].to(device)
                out_pre, out_fea, out_att, fea_dict = model(graph, use_type_eopch, use_type_eopch, mask, mix=args.mix, mae=args.mae)
                lbl_pred = out_pre[0]
...
def get_params():
    ...
    parser.add_argument("--mae", type=str, default='None', help="Experimental details")
