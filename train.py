...
from models.model_gns import fusion_model_mae
...
def train_a_epoch(model, train_data, all_data, patient_and_time, patient_sur_type, batch_size, optimizer, epoch, format_of_coxloss, args):
    ...
    for i_batch, id in enumerate(train_data):
        ...
        if len(args.train_use_type) == 1:
            ...
            graph = all_data[id].to(device)
            with torch.cuda.amp.autocast():
                out_pre, out_fea, out_att, fea_dict = model(graph, use_type_eopch, use_type_eopch, mask, mask1, mask2, mix=args.mix)
                lbl_pred = out_pre[0]
...
def main(args):
    ...
    for seed in range(start_seed, start_seed + repeat_num):
        ...
        for train_index, test_index in kf.split(patients, kf_label):
            ...
            torch.save(t_model.state_dict(),'../results/BRCA/our/' + sys_time.strftime('%Y-%m-%d') + label + '_' + str(seed) + '_' + str( n_fold) + '.pth')
    ...
    joblib.dump(all_gnn_time, '../results/BRCA/our/' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')
    joblib.dump(all_each_model_time, '../results/BRCA/our/' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')
    joblib.dump(all_all_ci, '../results/BRCA/our/' + 'all' + sys_time.strftime('%Y-%m-%d-%H-%M') + label + '.pkl')
